...
class MgcServer:
    ...
    TOOLS = [
        ...
        {
            "name": "mgc_get",
            "description": "Retrieve sensitive information or execute stored scripts. Use action=run to execute scripts (info_type must be 'script'). Use mgc_seal to generate sealed capsule for trusted nodes.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "info_type": {
        ...
        },
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "ext04": {
                        "type": "string",
                        "description": "Target node's public key in PEM format (required for seal)"
                    }
                },
    ...
    def _do_save(self, args: Dict[str, Any]) -> ToolResult:
        """Execute mgc_save"""
        # Filter out None values and reserved MCP field "name", pass all others to API
        data = {
        ...
        if not data.get("info_type") or not data.get("info_owner"):
            return ToolResult(success=False, content="", hint="Missing info_type or info_owner")
    ...
    def _do_get(self, args: Dict[str, Any]) -> ToolResult:
        """Execute mgc_get"""
        # Pass all parameters like mgc_save does
        data = {
            k: v for k, v in args.items()
            if v is not None and k != "name"
        }
        ...
        if isinstance(response, str):
            return ToolResult(
                success=False,
                content=response,
                error="API returned string instead of dict"
            )
    ...
    def _do_seal(self, args: Dict[str, Any]) -> ToolResult:
        ...
        if not data.get("info_owner"):
            return ToolResult(
                success=False,
                content="",
                error="Missing required param: info_owner"
            )
        if not data.get("ext04") and not data.get("ext04"):
            return ToolResult(
                success=False,
                content="",
                error="Missing required param: ext04 (node public key). Get node_pub first with mgc_get)."
            )
